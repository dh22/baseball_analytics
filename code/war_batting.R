...
download.file(fileUrl, destfile="./raw/war_daily_bat.csv", method="curl")

#Write the download to a data frame
df <- read.csv("./raw/war_daily_bat.csv", header=TRUE)
...
#Reorder data frame
final <- subset(df3, select = c(playerID,player_ID,name_common,nameFirst,nameLast,year_ID, age, team_ID, stint_ID, lg_ID, PA, G, 
...
save(war_batting,file="./trn/war_batting.RData")
