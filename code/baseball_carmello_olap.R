...
pitching2 <-  full_join(pitching_sum,pitching_avg,by=group) %>% mutate(.,W_Pct= W/(W+L),Inn = IPouts/3,PlayerYearID=paste(playerID,"-",yearID,sep="")) %>% select(.,-IPouts)
...
batting3<-batting2%>%select(.,-playerID, -yearID) #%>% group_by(.,PlayerYearID) %>% mutate_each(funs(replace(., is.na(.), 0)))
pitching3<-pitching2%>%select(.,-playerID, -yearID)#%>% group_by(.,PlayerYearID) %>% mutate_each(funs(replace(., is.na(.), 0)))
