...
olap_actuals <- olap
olap<-olap%>% filter(.,yearID <=1997) #Cuts off data for hindcasting

#parameters

player_select <-  "Alex Rodriguez"#"Edwin Encarnacion" #"Robinson Cano" #"Aroldis Chapman"  #"Mike Trout"#"Matt Kemp"#"Billy Hamilton" 
...
##testing projecting war instead of % change
...
war_prj<- select(hist_player_career,yearSeason,Similarity_Score)%>% group_by(.,yearSeason) %>%summarise(.,sim_sum=sum(Similarity_Score)) %>% left_join(select(hist_player_career,yearSeason,WAR,Similarity_Score),.) %>%mutate(.,sim_weight=Similarity_Score/sim_sum)%>% group_by(.,yearSeason) %>%select(.,-Similarity_Score,-sim_sum ) %>% mutate(.,n=n(),s=sd(WAR))%>% mutate(.,trim.lo=floor(n*trim)+1)%>%mutate(.,trim.hi=n+1-trim.lo) %>% arrange(.,yearSeason,WAR)%>% mutate(.,groupcount=index(n))%>% filter(.,groupcount>=trim.hi & groupcount<=trim.lo)
...
#  wttest <-wtd.t.test(war_prj$WAR_pct_chg[which(war_prj$yearSeason==i)],weight=war_prj$sim_weight[which(war_prj$yearSeason==i)])
  wttest <-wtd.t.test(war_prj$WAR[which(war_prj$yearSeason==i)],weight=war_prj$sim_weight[which(war_prj$yearSeason==i)])
  
  prob= 1-((1-confidence_level)/2)
  
  #pct_chg_mean=wttest$additional[[2]]
  WAR2=wttest$additional[[2]]
  ...
  war_prj$WAR2=ifelse(war_prj$yearSeason==i,WAR2,war_prj$WAR2)
...
war_player<-select(war_player,-WAR.y, -trim.lo, -trim.hi,-groupcount, -sim_weight)%>% mutate(.,WAR=coalesce(WAR.x,WAR2),upper=ifelse(d_forecast==1,upper,NA),lower=ifelse(d_forecast==1,lower,NA)) %>% mutate(.,projected_value=WAR*WAR_Value)
