...
player_list <-  sample(olap[which(olap$yearID==hindcast_year & olap$primary_pos!="P"),]$nameFirstLast,1,replace=F)
...
war_player<-select(war_player,-WAR_pct_chg, -trim.lo, -trim.hi,-groupcount, -sim_weight)%>%group_by(.,yearSeason)%>% mutate(.,war_sum=WAR2+comp_avg_war) %>%mutate(.,WAR_Comp=(.60*WAR2)+(.40*comp_avg_war))%>%mutate(.,WAR=coalesce(WAR,WAR_Comp),upper=ifelse(d_forecast==1,upper,NA),lower=ifelse(d_forecast==1,lower,NA)) %>% mutate(.,projected_value=WAR*WAR_Value) %>% ungroup(.)
...
ggplot(war_player,aes(x=yearID))+
  ...
  scale_y_continuous(limits=c(-5,15),breaks = seq(-5,15,by = 3))+
