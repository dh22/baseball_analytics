...
batting_sum <- 
  batting %>% 
  select(.,one_of(group),one_of(battinglist_sum)) %>%
...
batting_avg <- 
  batting %>%
  select(.,one_of(group),one_of(battinglist_avg)) %>%
  group_by(.,playerID,yearID) %>%
  summarise_each(.,funs(mean(.,na.rm = T))) %>%
  ungroup(.) %>%
  mutate(.,ISO = SlugPct-BA)

batting2 <-
  full_join(batting_sum,batting_avg,by=group) %>%
  mutate(.,PlayerYearID=paste(playerID,"-",yearID,sep=""))
...
pitching_sum <- 
  pitching %>% 
  select(., one_of(group),one_of(pitchinglist_sum)) %>%
  group_by(.,playerID,yearID) %>%
  summarise_each(.,funs(sum(.,na.rm=T))) %>%
  ungroup(.)
...
pitching_avg <-
  pitching %>%
  select(., one_of(group),one_of(pitchinglist_avg)) %>%
  group_by(.,playerID,yearID) %>%
  summarise_each(.,funs(mean(.,na.rm=T))) %>%
  ungroup(.)

pitching2 <- 
  full_join(pitching_sum,pitching_avg,by=group) %>%
  mutate(.,W_Pct=W/(W+L),Inn = IPouts/3,PlayerYearID=paste(playerID,"-",yearID,sep=""),BB_9=(BB*9)/(IPouts/3),SO_9=(SO*9)/(IPouts/3)) %>%
  mutate(.,FIP=((13*H) + (3*BB) - 2*SO) / Inn) %>%
  select(.,-IPouts)
...
player_pos <- 
  select(Fielding,playerID,POS,G) %>% 
  filter(.,!POS %in% c("RF","CF","LF")) %>% 
  group_by(.,playerID,POS) %>% 
  summarise(.,psum=sum(G,na.rm=T)) %>% 
  ungroup(.) %>%
  arrange(.,playerID,desc(psum)) %>%
  group_by(.,playerID) %>%
  mutate(.,primary_pos = first(POS,order_by = c("playerID"))) %>%
  select(.,-POS,-psum) %>%
  distinct(.,.keep_all=T) %>%
  ungroup(.)
...
btt_weight <- c(16,19,8,8,14,16,19)#c(10,12,2,2,8,10,12) #DH -Updated 11/8/16 previous weights were incorrrect ##added btt_WAR_def
btt_weight <-  btt_weight/sum(btt_weight)
btt_sts <- c("G","AB","BA","SlugPct","OBP","ISO","WAR_def") ##added btt_WAR_def
map_btt_weight <- 
  data.frame(btt_sts,btt_weight) %>%
  spread(.,btt_sts,btt_weight)
...
ptch_weight <- c(8,13,13,11,11,10,17,17) #c(20,10,50,10,10,5,5,50) #DH -Updated 11/1/16 previous weights were incorrrect
ptch_weight <- ptch_weight/sum(ptch_weight)
ptch_sts <- c("G","GS","Inn","SO_9","BB_9","SV","FIP","ERA_plus") 
map_ptch_weight <- 
  data.frame(ptch_sts,ptch_weight) %>%
  spread(.,ptch_sts,ptch_weight)
...
war_batting2 <- 
  war_batting %>%
  select(.,playerID,BBRef_playerID,yearID,pitcher,WAR,WAR_def) %>%
  group_by(.,playerID,BBRef_playerID,yearID,pitcher) %>% 
  summarise_each(.,funs(sum(.,na.rm=T))) %>% 
  ungroup(.)

war_pitching2 <-
  war_pitching %>%
  select(.,playerID,BBRef_playerID,yearID,WAR,ERA_plus) %>%
  group_by(.,playerID,BBRef_playerID,yearID) %>% 
  summarise(.,WAR=sum(WAR,na.rm=T),ERA_plus=mean(ERA_plus,na.rm=T)) %>% 
  ungroup(.)
...
war_all <-  
  mutate(war_all,PlayerYearID=paste(playerID,yearID,sep="-")) %>%
  select(., -WAR.x, -WAR.y) %>% 
  rename(.,btt_WAR_def=WAR_def,ptch_ERA_plus=ERA_plus) ##added btt_WAR_def
...
players <- 
  left_join(players,player_yrs) %>%
  mutate(.,PlayerYearID=paste(playerID,"-",yearID,sep=""),nameFirstLast=paste(nameFirst,nameLast),BBRefYearID=paste(bbrefID,"-",yearID,sep="")) %>%
  mutate(.,season_start =as.Date(paste(as.character(yearID),"04","01",sep="-")),birthDate=as.Date(birthDate)) %>% 
  mutate(.,age=as.numeric(round((difftime(season_start,birthDate,unit="weeks")/52.25),1))) %>% 
  left_join(.,player_pos,by="playerID") %>% 
  select(.,-season_start)
...
final_ptch_vars<-  paste("ptch_",c("G","GS","Inn","SV","BB_9","SO_9","FIP"),sep="")
#drop playerid and yearid 

batting3 <- 
  batting2 %>%
  select(.,PlayerYearID,one_of(final_btt_vars)) #%>% group_by(.,PlayerYearID) %>% mutate_each(funs(replace(., is.na(.), 0)))

pitching3 <- 
  pitching2 %>% 
  select(.,PlayerYearID,one_of(final_ptch_vars))#%>% group_by(.,PlayerYearID) %>% mutate_each(funs(replace(., is.na(.), 0)))

war_all2 <- war_all %>%
  select(.,PlayerYearID,WAR,btt_WAR_def,ptch_ERA_plus)#%>% group_by(.,PlayerYearID) %>% mutate_each(funs(replace(., is.na(.), 0))) ##added btt_WAR_def

#Join Datasets
olap <- 
  left_join(players,batting3, by="PlayerYearID") %>%
  left_join(.,pitching3, by="PlayerYearID") %>%
  left_join(.,war_all2,by="PlayerYearID") %>%
  merge(.,map_btt_weight) %>%
  merge(.,map_ptch_weight) %>%
  left_join(.,map_pos_weight,by=c("primary_pos"="pos")) 
