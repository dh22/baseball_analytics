...
war_prj<- select(hist_player_career,yearSeason,Similarity_Score)%>% group_by(.,yearSeason) %>% summarise(.,sim_sum=sum(Similarity_Score)) %>% left_join(select(hist_player_career,yearSeason,WAR_pct_chg,Similarity_Score),.) %>%mutate(.,sim_weight=Similarity_Score/sim_sum)%>% group_by(.,yearSeason) %>%select(.,-Similarity_Score,-sim_sum ) %>% mutate(.,n=n(),s=sd(WAR_pct_chg))%>%select(.,-s)%>% group_by(.,yearSeason)%>% summarise(.,avg_war_pct_chg=weighted.mean(WAR_pct_chg,sim_weight)) %>%ungroup(.) %>%filter(.,yearSeason>=1)
...
war_player <- war_player %>% mutate(.,avg_mape=avg_mape)%>% mutate(.,upper=ifelse(d_forecast==1,WAR*(1+(avg_mape*2)),NA), lower=ifelse(d_forecast==1,WAR*(1-(avg_mape*2)),NA))
